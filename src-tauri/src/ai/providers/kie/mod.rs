...
impl KieProvider {
    ...
    fn is_video_model(model: &str) -> bool {
        matches!(
            model,
            "wan-2-7-i2v" | "wan-2-7-t2v" | "wan-2-7-r2v" | "seedance-2" | "seedance-2-fast"
        )
    }

    fn resolve_api_model_name(model: &str) -> String {
        match model {
            "wan-2-7-i2v" => "wan/2-7-image-to-video".to_string(),
            "wan-2-7-t2v" => "wan/2-7-text-to-video".to_string(),
            "wan-2-7-r2v" => "wan/2-7-r2v".to_string(),
            "seedance-2" => "bytedance/seedance-2".to_string(),
            "seedance-2-fast" => "bytedance/seedance-2-fast".to_string(),
            "gpt-image-2-text-to-image" => "gpt-image-2-text-to-image".to_string(),
            "gpt-image-2-image-to-image" => "gpt-image-2-image-to-image".to_string(),
    ...
    async fn create_task(
        ...
        let api_model = Self::resolve_api_model_name(model);
        let input = if Self::is_video_model(model) {
            Self::build_video_input(request, model, &uploaded_images)
        } else {
            Self::build_image_input(request, model, &uploaded_images)?
        };

        info!(
            "[KIE createTask] model={}, api_model={}, is_video={}",
            model, api_model, Self::is_video_model(model)
        );
        ...
        match model {
            "wan-2-7-i2v" => {
                ...
                if !first_frame_url.is_empty() {
                    input["first_frame_url"] = json!(first_frame_url);
                } else if !uploaded_images.is_empty() {
                    input["first_frame_url"] = json!(uploaded_images[0]);
                }
            ...
            "wan-2-7-r2v" => {
                ...
                // R2V requires reference_image parameter
                if !uploaded_images.is_empty() {
                    input["reference_image"] = json!(uploaded_images);
                }
                input
            }
...
impl AIProvider for KieProvider {
    ...
    async fn submit_task(&self, mut request: GenerateRequest) -> Result<ProviderTaskSubmission, AIError> {
        let api_key = self
            .api_key
            .read()
            .await
        ...
        // For video models, upload first_frame_url from extra_params if present
        if Self::is_video_model(&model) {
            if let Some(ref mut extra) = request.extra_params {
                if let Some(frame_val) = extra.get("first_frame_url").cloned() {
                    if let Some(frame_source) = frame_val.as_str() {
                        if !frame_source.is_empty() && !Self::is_http_url(frame_source) {
                            let uploaded_url = self.upload_reference_image(&api_key, frame_source, 0).await?;
                            extra.insert("first_frame_url".to_string(), json!(uploaded_url));
                        }
                    }
                }
            }
