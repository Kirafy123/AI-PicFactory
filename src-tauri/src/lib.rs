...
pub fn run() {
    ...
    tauri::Builder::default()
        ...
        .setup(|app| {
            ...
            // Load custom provider state from SQLite (best-effort).
            {
                let provider = custom_provider::custom_provider_handle();
                if let Err(err) = custom_provider::load_state_from_db(&provider, &app_handle) {
                    warn!("failed to load custom provider state: {err}");
                }
            }
        ...
        .invoke_handler(tauri::generate_handler![
            ...
            custom_provider::configure_custom_provider,
            custom_provider::get_custom_provider_config,
            custom_provider::list_custom_provider_targets,
            custom_provider::set_custom_supported_models,
