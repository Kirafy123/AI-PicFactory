...
export const StoryboardGenNode = memo(({ id, data, selected, width, height }: StoryboardGenNodeProps) => {
  ...
  const customTargetProviderId = useSettingsStore((state) => state.customTargetProviderId);
  const customSupportedModels = useSettingsStore((state) => state.customSupportedModels);
  ...
  const imageModels = useMemo(
    () => listImageModels(),
    [customTargetProviderId, customSupportedModels]
  );
  ...
  const effectiveExtraParams = useMemo(
    () => ({
      ...
      ...(selectedModel.id === GRSAI_NANO_BANANA_PRO_MODEL_ID
        ? { grsai_pro_model: 'nano-banana-pro' }
        : {}),
    }),
    [nodeData.extraParams, selectedModel.id]
  );
