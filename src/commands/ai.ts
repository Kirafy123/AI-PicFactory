...
const FALLBACK_TARGETS = ['grsai', 'kie'];
const FALLBACK_CONFIG: CustomProviderConfig = {
  target_provider_id: 'grsai',
  supported_models: ['nano-banana-pro', 'nano-banana-2'],
};

export async function configureCustomProvider(targetProviderId: string): Promise<void> {
  console.info('[AI] configure_custom_provider', { targetProviderId });
  if (!isTauri()) return;
  await invoke('configure_custom_provider', { targetProviderId });
}

export async function getCustomProviderConfig(): Promise<CustomProviderConfig> {
  if (!isTauri()) return { ...FALLBACK_CONFIG };
  return await invoke<CustomProviderConfig>('get_custom_provider_config');
}

export async function listCustomProviderTargets(): Promise<string[]> {
  if (!isTauri()) return [...FALLBACK_TARGETS];
  return await invoke<string[]>('list_custom_provider_targets');
}

export async function setCustomSupportedModels(
  targetProviderId: string,
  models: string[]
): Promise<CustomProviderConfig> {
  if (!isTauri()) return { ...FALLBACK_CONFIG, supported_models: models };
  return await invoke<CustomProviderConfig>('set_custom_supported_models', {
    targetProviderId,
    models,
  });
}
